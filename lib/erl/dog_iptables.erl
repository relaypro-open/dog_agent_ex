...
%-include_lib("amqp_client_internal.hrl").
%-include_lib("amqp_gen_consumer_spec.hrl").
...
apply_ipv6_ruleset(TempFile) ->
    ...
    case Enforcing of
      ...
      Result = dog_os:cmd(Cmd),
      case Result of
        [] -> io:format("applied ipv6 ruleset"), ok;
        _ ->
        io:format("validate_ipv6_ruleset Result: ~p",
                [Result]),
        error
      end
    end.
...
backup_current_ipv4_iptables() ->
    ...
    Result = dog_os:cmd(Cmd),
    io:format("backup_ipv4_iptables Result: ~p",
        [Result]),
    case Result of
      [] -> ok;
      _ -> error
...
backup_current_ipv6_iptables() ->
    ...
    Result = dog_os:cmd(Cmd),
    io:format("backup_ipv6_iptables Result: ~p",
        [Result]),
    case Result of
      [] -> ok;
      _ -> error
...
update_iptables4(Ruleset, Retry) ->
    ...
    case apply_ipv4_ruleset(TempFile) of
      ...
      case Retry of
        ...
        io:format("Retry count updating IPv4 iptables: ~p",
               [R + 1]),
        update_iptables4(Ruleset, R + 1);
        R when R > IptablesRestoreRetryLimit ->
        io:format("Unable to restore iptables after retry "
                "number: ~p",
                [R])
      end
    end.
...
update_iptables6(Ruleset, Retry) ->
    ...
    case apply_ipv6_ruleset(TempFile) of
      ...
      case Retry of
        R when R =< IptablesRestoreRetryLimit ->
        timer:sleep(IptablesRestoreRetryWaitSeconds * 1000),
        io:format("Retry count updating IPv6 iptables: ~p",
               [R + 1]),
...
handle_callback(Ipsets, R4IpsetsRuleset,    
    ...
    case application:get_env(dog, use_ipsets, true) of
      ...
      case R4IpsetsRuleset of
        false ->
        io:format("No v4 ipset ruleset to apply"), pass;
        _ -> ok = update_iptables4(R4IpsetsRuleset)
      end,
      case R6IpsetsRuleset of
        false ->
        io:format("No v6 ipset ruleset to apply"), pass;
        _ -> ok = update_iptables6(R6IpsetsRuleset)
      end;
      false ->
      case R4IptablesRuleset of
        false ->
        io:format("No v4 iptables ruleset to apply"), pass;
        _ -> ok = update_iptables4(R4IptablesRuleset)
      end,
      case R6IptablesRuleset of
        false ->
        io:format("No v6 iptables ruleset to apply"), pass;
