...
ec2_availability_zone() ->
    ...
    case hackney:request(Method, Url, Headers, Payload, Options) of
        {error, _Error} ->
            %?LOG_ERROR("Error getting ec2_availability_zone"),
            <<"">>;
...
ec2_security_group_ids(Mac) ->
    ...
    case hackney:request(Method, Url, Headers, Payload, Options) of
        {error, _Error} ->
            %?LOG_ERROR("Error getting ec2_security_group_ids"),
            [];
        {ok, StatusCode, _RespHeaders, ClientRef} ->
            case StatusCode of
                200 ->
...
ec2_public_ipv4(Mac) ->
    ...
    case hackney:request(Method, Url, Headers, Payload, Options) of
        {error, Error} ->
            %?LOG_ERROR("Error getting ec2_public_ipv4: ~p", [Error]),
            {error, failed};
        {ok, StatusCode, _RespHeaders, ClientRef} ->
            case StatusCode of
                200 ->
                ...
                404 ->
                    %?LOG_WARNING("Failed to get ec2_public_ipv4: ~p. This mac was not assigned a public-ipv4", [StatusCode]),
                    {error, notfound};
                _ ->
                    %?LOG_ERROR("Error getting ec2_public_ipv4: ~p", [StatusCode]),
                    {error, StatusCode}
            end
    end.
...
ec2_macs() ->
    ...
    case hackney:request(Method, Url, Headers, Payload, Options) of
        {error, _Error} ->
            %?LOG_ERROR("Error getting ec2 macs"),
            {error, notfound};
        {ok, StatusCode, _RespHeaders, ClientRef} ->
            case StatusCode of
                200 ->
